...
const updateUser = (req, res) => {
    ...
    User.findByIdAndUpdate(req.user._id, { name: 'Ivan', about: 'tsarevich'}, { new: true })
        .then((user) => {
            if(!user) {
...
const updateAvatar = (req, res) => {
    User.findOneAndUpdate(req.user.id, { avatar: 'https://images.unsplash.com/photo-1620924049153-4d32fcbe88fe?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxzZWFyY2h8Nnx8bmV3fGVufDB8fDB8fA%3D%3D&auto=format&fit=crop&w=700&q=60' }, { new: true })
        .then((user) => {
            if(!user) {
                res.status(NOT_FOUND).send({ message: 'Пользователь не найден' });
            } else {
